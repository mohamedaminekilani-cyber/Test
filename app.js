...
function renderDriver(){
  const active=state.activeDelivery?state.orders.find(o=>o.id===state.activeDelivery):null;
  const jobs=state.orders.filter(o=>o.status==='ready'&&o.id!==state.activeDelivery);
  app.innerHTML='<div class="page driver-page"><div class="driver-top"><div><p class="eyebrow">Courier workspace</p><h1>Ready to deliver</h1></div><span class="online-toggle">● Online</span></div><div class="driver-map"><div class="map-pin pin-a"><span>↗</span></div><div class="map-pin pin-b"><span>⌂</span></div><div class="map-pin pin-c"><span>•</span></div></div><div class="driver-stats"><div class="driver-stat"><strong>38.5 DT</strong><small>Today’s earnings</small></div><div class="driver-stat"><strong>6</strong><small>Deliveries</small></div><div class="driver-stat"><strong>4.96 ★</strong><small>Driver rating</small></div></div>'+(active?'<div class="panel"><div class="panel-head"><h3>Active delivery · '+active.id+'</h3><span class="status-pill delivering">'+statusLabel(active.status)+'</span></div><div class="route"><div class="route-line"><span class="route-dot"></span><div><strong>'+vendor(active.vendorId).name+'</strong><small>Pickup · '+vendor(active.vendorId).area+'</small></div><span>Pickup</span></div><div class="route-line"><span class="route-dot end"></span><div><strong>'+active.address+'</strong><small>Customer drop-off</small></div><span>Drop-off</span></div></div><div class="job-actions"><button class="primary-btn" onclick="advanceDelivery(\''+active.id+'\')">'+(active.status==='ready'?'Confirm pickup':'Complete delivery')+'</button></div></div>':'')+'<section class="section"><div class="section-head"><div><h2>Available jobs</h2><p>'+jobs.length+' courier-ready orders near you.</p></div></div><div class="job-list">'+(jobs.length?jobs.map(driverJob).join(''):'<div class="empty-state"><span>✓</span><h3>All caught up</h3><p>New jobs appear when a seller marks an order ready.</p></div>')+'</div></section></div>';
}
function driverJob(o){
  const maker=vendor(o.vendorId);
  return '<article class="job-card"><div class="job-top"><div><small>'+o.id+' · '+o.items.reduce((s,i)=>s+i.qty,0)+' package(s)</small><h3>'+maker.name+'</h3></div><div class="job-pay">'+money(o.fee+4)+'</div></div><div class="route"><div class="route-line"><span class="route-dot"></span><div><strong>'+maker.area+'</strong><small>Pickup from maker</small></div><span>Now</span></div><div class="route-line"><span class="route-dot end"></span><div><strong>'+o.address+'</strong><small>Customer drop-off</small></div><span>~22 min</span></div></div><div class="job-actions"><button class="ghost-btn" onclick="showToast(\'Route preview opened\')">Route</button><button class="primary-btn" onclick="acceptDelivery(\''+o.id+'\')">Accept '+money(o.fee+4)+'</button></div></article>';
}
function acceptDelivery(id){if(state.activeDelivery){showToast('Complete the active delivery first');return}state.activeDelivery=id;save();showToast('Delivery '+id+' accepted');render()}
function advanceDelivery(id){
  const o=state.orders.find(x=>x.id===id);if(!o)return;
  if(o.status==='ready'){o.status='delivering';showToast('Pickup confirmed');}
  else if(o.status==='delivering'){o.status='delivered';state.activeDelivery='';showToast('Delivery completed');}
  save();render();
}
...
function navigate(view){currentMode='buyer';currentView=view;closeDrawers();render();window.scrollTo({top:0,behavior:'smooth'})}
function render(){
  updateHeader();
...
document.querySelectorAll('[data-action="mode"]').forEach(btn=>btn.addEventListener('click',()=>openDrawer('mode')));
document.getElementById('cartBtn').addEventListener('click',()=>openDrawer('cart'));
document.getElementById('profileBtn').addEventListener('click',()=>openDrawer('mode'));
document.getElementById('searchBtn').addEventListener('click',openSearch);
document.getElementById('locationBtn').addEventListener('click',openLocation);
scrim.addEventListener('click',closeDrawers);
document.querySelectorAll('[data-close="cart"],[data-close="mode"]').forEach(btn=>btn.addEventListener('click',closeDrawers));
...
/* ===== PantryMade Pro expansion ===== */
let marketSort='popular';
let deliverySlot='ASAP';
let driverFilter='best';
let selectedDriverJobs=[];
let routeMap=null;
let routeLayer=null;
let routeMarkers=[];
let routeMetrics={km:0,min:0};
let adminTab='overview';

function upgradeData(){
  const vendorMeta={
    v1:{lat:36.8665,lng:10.1647,hours:'08:00–18:00',license:'Verified',response:'6 min',minimum:15,deliveryRadius:10,commission:12},
    v2:{lat:36.8065,lng:10.1815,hours:'09:00–19:00',license:'Verified',response:'8 min',minimum:12,deliveryRadius:12,commission:12},
    v3:{lat:36.8782,lng:10.3247,hours:'10:00–20:00',license:'Verified',response:'4 min',minimum:10,deliveryRadius:14,commission:12},
    v4:{lat:36.8531,lng:10.3233,hours:'08:30–17:30',license:'Verified',response:'11 min',minimum:15,deliveryRadius:11,commission:12}
  };
  state.vendors.forEach(function(x){Object.assign(x,vendorMeta[x.id]||{});if(!x.reviewCount)x.reviewCount=x.reviews||80;if(!x.fulfilment)x.fulfilment=97;});
  const moreVendors=[
  ...
  state.products.forEach(function(x){if(!x.tags)x.tags=[x.category];if(!x.rating)x.rating=4.8;if(!x.reviews)x.reviews=20+Math.floor(Math.random()*80);if(!x.ingredients)x.ingredients='Prepared in small batches with clearly labeled pantry ingredients.';});
  moreProducts.forEach(function(x){x.rating=x.rating||4.8;x.reviews=x.reviews||30;x.ingredients=x.ingredients||'Small-batch pantry ingredients with full label details.';if(!state.products.some(function(p){return p.id===x.id;}))state.products.push(x);});
  const extraOrders=[
  ...
  const orderCoords={
    'PM-1048':[36.8480,10.1870],'PM-1047':[36.8391,10.1622],'PM-1046':[36.8450,10.1950]
  };
  state.orders.forEach(function(o){
    ...
    if((!o.lat||!o.lng)&&orderCoords[o.id]){o.lat=orderCoords[o.id][0];o.lng=orderCoords[o.id][1];}
...
function orderStage(status){
  const map={new:0,accepted:1,preparing:1,ready:2,driver_assigned:3,delivering:3,delivered:4,rejected:0};
  return map[status]||0;
}
function orderItemCount(o){return (o.items||[]).reduce(function(s,i){return s+(i.qty||1);},0);}
function orderItemNames(o){
  return (o.items||[]).map(function(i){const p=product(i.productId);return (i.qty||1)+'× '+(p?p.name:'Item');}).join(', ');
}
function productCard(p){
  const maker=vendor(p.vendorId);
  const fav=state.favorites.includes(p.id);
  return '<article class="product-card" onclick="openProductDetail(\''+p.id+'\')"><div class="product-media"><span class="badge">'+p.badge+'</span><button class="heart" onclick="event.stopPropagation();toggleFavorite(\''+p.id+'\')">'+(fav?'♥':'♡')+'</button><div class="food-art">'+p.emoji+'</div></div><div class="product-body"><button class="vendor-line" style="border:0;background:none;padding:0;cursor:pointer" onclick="event.stopPropagation();openStore(\''+maker.id+'\')"><span class="verified">●</span>'+maker.name+' · ★ '+p.rating+'</button><h3>'+p.name+'</h3><div class="product-tags">'+(p.tags||[p.category]).slice(0,3).map(function(t){return '<span class="tag-mini">'+t+'</span>';}).join('')+'</div><div class="product-meta"><span>'+p.unit+'</span><span>'+p.shelf+'</span><span>'+p.stock+' left</span></div><div class="price-row"><span class="price">'+money(p.price)+'</span><button class="add-btn" onclick="event.stopPropagation();addToCart(\''+p.id+'\')">+</button></div></div></article>';
}
function storeCard(v){
  return '<article class="store-card" onclick="openStore(\''+v.id+'\')"><div class="store-cover">'+v.cover+'</div><div class="store-avatar">'+v.initials+'</div><h3>'+v.name+'</h3><small>'+v.category+' · '+v.hours+'</small><div class="store-stats"><span>★ '+v.rating+' ('+v.reviewCount+')</span><span>'+v.area+'</span><span>'+v.eta+'</span></div><div class="store-health"><span>'+v.fulfilment+'% fulfil</span><span>'+v.response+' reply</span><span>'+v.minimum+' DT min</span></div></article>';
}
function openProductDetail(id){
  const p=product(id),maker=vendor(p.vendorId);
  openModal('<div style="text-align:center;font-size:64px">'+p.emoji+'</div><p class="eyebrow">'+maker.name+'</p><h2>'+p.name+'</h2><p>'+p.ingredients+'</p><div class="detail-grid"><div class="detail-box"><b>Pack & price</b><span>'+p.unit+' · '+money(p.price)+'</span></div><div class="detail-box"><b>Shelf life</b><span>'+p.shelf+'</span></div><div class="detail-box"><b>Allergens</b><span>'+p.allergens+'</span></div><div class="detail-box"><b>Stock</b><span>'+p.stock+' units available</span></div></div><div class="product-tags">'+(p.tags||[]).map(function(t){return '<span class="tag-mini">'+t+'</span>';}).join('')+'</div><p>★ '+p.rating+' from '+p.reviews+' reviews · Maker fulfilment '+maker.fulfilment+'%</p><div class="modal-actions"><button class="ghost-btn" onclick="closeModal();openStore(\''+maker.id+'\')">Visit store</button><button class="primary-btn" onclick="addToCart(\''+p.id+'\');closeModal()">Add to basket · '+money(p.price)+'</button></div>');
}
function setMarketSort(v){marketSort=v;render();}
function setDeliverySlot(v){deliverySlot=v;render();}
function renderMarketplace(){
  const categories=['All'].concat(Array.from(new Set(state.products.map(function(p){return p.category;}))));
  let list=state.products.filter(function(p){return selectedCategory==='All'||p.category===selectedCategory;});
  if(currentSearch){
    list=list.filter(function(p){const m=vendor(p.vendorId);return [p.name,p.category,p.allergens,(p.tags||[]).join(' '),m.name,m.category].join(' ').toLowerCase().includes(currentSearch.toLowerCase());});
  }
  ...
  const slots=['ASAP · 25–45 min','18:00–19:00','19:00–20:00','Tomorrow 10–12'];
  app.innerHTML='<div class="page"><section class="hero"><div class="hero-main"><span class="hero-kicker">● '+state.vendors.length+' verified makers · '+state.products.length+' pantry items</span><h1>Homemade pantry food, organized like a real marketplace.</h1><p>Shop across independent home kitchens, schedule delivery, follow each seller order separately, and let one courier route multiple pickups efficiently.</p><div class="hero-actions"><button class="secondary-btn" onclick="scrollToProducts()">Browse '+state.products.length+' items</button><button class="primary-btn" onclick="switchMode(\'vendor\')">Open seller studio</button></div></div><div class="hero-side"><div class="hero-tile"><span class="tile-icon">📦</span><div><strong>Multi-store checkout</strong><p>One basket, separate maker tickets, one delivery experience.</p></div></div><div class="hero-tile dark"><span class="tile-icon">🗺️</span><div><strong>Smart courier routes</strong><p>Batch-ready orders can be sequenced and mapped automatically.</p></div></div></div></section><section class="section"><div class="quick-grid"><div class="quick-card"><span>✓</span><div><strong>Verified maker profiles</strong><small>Store hours, response time, fulfilment and product labels.</small></div></div><div class="quick-card"><span>🧾</span><div><strong>Detailed order tickets</strong><small>Notes, payment, prep time and customer contact.</small></div></div><div class="quick-card"><span>🛵</span><div><strong>Batch delivery</strong><small>Drivers can group compatible pickups and drops.</small></div></div><div class="quick-card"><span>📍</span><div><strong>Mapped coverage</strong><small>Tunis-area demo coordinates with road routing.</small></div></div></div></section><section class="section"><div class="section-head"><div><h2>Delivery window</h2><p>Choose when the basket should arrive.</p></div></div><div class="delivery-strip">'+slots.map(function(s){return '<button class="delivery-slot '+(deliverySlot===s||deliverySlot==='ASAP'&&s.indexOf('ASAP')===0?'is-active':'')+'" onclick="setDeliverySlot(\''+s.replace(/'/g,"\'")+'\')"><b>'+s+'</b><small>'+(s.indexOf('ASAP')===0?'Fastest available':'Reserve this window')+'</small></button>';}).join('')+'</div></section><section class="section"><div class="section-head"><div><h2>Shop the pantry</h2><p>Search, filter and compare independent makers.</p></div></div><div class="market-toolbar"><input value="'+currentSearch.replace(/"/g,'&quot;')+'" oninput="currentSearch=this.value;render()" placeholder="Search harissa, granola, vegan, maker…"><select onchange="selectedCategory=this.value;render()">'+categories.map(function(c){return '<option '+(selectedCategory===c?'selected':'')+'>'+c+'</option>';}).join('')+'</select><select onchange="setMarketSort(this.value)"><option value="popular" '+(marketSort==='popular'?'selected':'')+'>Most popular</option><option value="rating" '+(marketSort==='rating'?'selected':'')+'>Top rated</option><option value="price" '+(marketSort==='price'?'selected':'')+'>Lowest price</option></select></div><div class="product-grid" id="productGrid">'+(list.length?list.map(productCard).join(''):'<div class="empty-state" style="grid-column:1/-1"><span>⌕</span><h3>No matches</h3><p>Try another search or category.</p></div>')+'</div></section><section class="section"><div class="section-head"><div><h2>Featured home kitchens</h2><p>Operational quality is visible before you order.</p></div><button class="ghost-btn" onclick="navigate(\'stores\')">All '+state.vendors.length+' stores</button></div><div class="store-grid">'+state.vendors.slice(0,6).map(storeCard).join('')+'</div></section><div class="info-banner"><span>ⓘ</span><span><strong>Demo operations:</strong> products, orders, payments and routes are simulated in-browser. Production launch requires a backend, identity/compliance checks and locally valid home-food rules.</span></div></div>';
}
function renderStores(){
...
function renderStore(){
  const v=vendor(selectedStore)||state.vendors[0];
  const list=state.products.filter(function(p){return p.vendorId===v.id;});
  app.innerHTML='<div class="page"><section class="store-hero"><div class="store-avatar">'+v.initials+'</div><div><p class="eyebrow" style="color:#bcd2c4">'+v.license+' home maker</p><h1>'+v.name+'</h1><p>'+v.bio+'</p><div class="store-stats"><span>★ '+v.rating+' ('+v.reviewCount+')</span><span>'+v.area+'</span><span>'+v.eta+'</span><span>'+v.hours+'</span></div></div><button class="primary-btn" onclick="showToast(\'Following '+v.name+'\')">♡ Follow</button></section><div class="quick-grid" style="margin-top:14px"><div class="quick-card"><span>⚡</span><div><strong>'+v.response+' response</strong><small>Typical seller response time.</small></div></div><div class="quick-card"><span>✓</span><div><strong>'+v.fulfilment+'% fulfilment</strong><small>Orders completed successfully.</small></div></div><div class="quick-card"><span>🧺</span><div><strong>'+v.minimum+' DT minimum</strong><small>Minimum store subtotal.</small></div></div><div class="quick-card"><span>📍</span><div><strong>'+v.deliveryRadius+' km coverage</strong><small>Demo service radius.</small></div></div></div><section class="section"><div class="section-head"><div><h2>From this kitchen</h2><p>'+list.length+' active products.</p></div></div><div class="product-grid">'+list.map(productCard).join('')+'</div></section></div>';
}
function timelineHtml(o){
  const labels=['Order placed','Seller preparing','Ready for courier','Out for delivery','Delivered'];
...
function renderOrders(){
  app.innerHTML='<div class="page"><div class="section-head"><div><p class="eyebrow">Buyer account</p><h2>Orders & live fulfilment</h2><p>Each maker ticket advances independently before courier consolidation.</p></div></div><div class="order-card-grid">'+state.orders.slice().sort(function(a,b){return orderStage(b.status)-orderStage(a.status);}).map(function(o){const maker=vendor(o.vendorId),stage=orderStage(o.status);return '<article class="order-card"><div class="order-card-top"><div><p class="eyebrow">'+o.id+' · '+o.created+'</p><h3>'+maker.emoji+' '+maker.name+'</h3></div><span class="status-pill '+statusClass(o.status)+'">'+statusLabel(o.status)+'</span></div><div class="order-items">'+(o.items||[]).map(function(i){const p=product(i.productId);return '<div class="order-item-line"><span>'+(i.qty||1)+'× '+(p?p.name:'Item')+'</span><b>'+(p?money(p.price*(i.qty||1)):'')+'</b></div>';}).join('')+'</div><div class="order-progress">'+[0,1,2,3,4].map(function(i){return '<span class="progress-step '+(i<=stage?'done':'')+'"></span>';}).join('')+'</div><div class="product-meta"><span>'+o.payment+'</span><span>'+o.address+'</span><span>'+money(o.total)+'</span></div><div class="order-actions"><button class="ghost-btn" onclick="openBuyerOrder(\''+o.id+'\')">Order details</button>'+(o.status==='delivered'?'<button class="primary-btn" onclick="reorder(\''+o.id+'\')">Reorder</button>':'<button class="primary-btn" onclick="showToast(\'Support thread opened\')">Message</button>')+'</div></article>';}).join('')+'</div></div>';
}
function openBuyerOrder(id){
  const o=state.orders.find(function(x){return x.id===id;}),m=vendor(o.vendorId);
  openModal('<p class="eyebrow">'+o.id+'</p><h2>'+m.name+' order</h2><p>'+orderItemNames(o)+'</p>'+timelineHtml(o)+'<div class="detail-grid"><div class="detail-box"><b>Delivery</b><span>'+o.address+'</span></div><div class="detail-box"><b>Payment</b><span>'+o.payment+'</span></div><div class="detail-box"><b>Customer note</b><span>'+o.notes+'</span></div><div class="detail-box"><b>Total</b><span>'+money(o.total)+'</span></div></div><div class="modal-actions"><button class="ghost-btn" onclick="showToast(\'Support thread opened\')">Contact support</button><button class="primary-btn" onclick="closeModal()">Done</button></div>');
}
function reorder(id){
  const o=state.orders.find(function(x){return x.id===id;});
  (o.items||[]).forEach(function(i){const existing=state.cart.find(function(c){return c.productId===i.productId;});if(existing)existing.qty+=i.qty||1;else state.cart.push({productId:i.productId,qty:i.qty||1});});
  save();updateHeader();showToast('Order added back to basket');openDrawer('cart');
}

function sellerMetrics(){
  const mine=state.orders.filter(function(o){return o.vendorId==='v1';});
  const revenue=mine.filter(function(o){return o.status!=='rejected';}).reduce(function(s,o){return s+o.total;},0);
...
function renderVendor(){
  const m=sellerMetrics(),products=state.products.filter(function(p){return p.vendorId==='v1';});
  let content='';
  if(vendorTab==='overview'){
    content='<div class="metric-grid"><div class="metric"><small>Gross sales</small><strong>'+money(m.revenue)+'</strong><div class="trend">↑ 18% vs last week</div></div><div class="metric"><small>New orders</small><strong>'+m.newCount+'</strong><div class="trend">Accept within 10 minutes</div></div><div class="metric"><small>Ready handoffs</small><strong>'+m.ready+'</strong><div class="trend">Courier queue</div></div><div class="metric"><small>Avg. prep</small><strong>22 min</strong><div class="trend">↓ 3 min this week</div></div></div><div class="ops-grid"><div class="panel"><div class="panel-head"><h3>Live order board</h3><button class="mini-btn" onclick="vendorTab=\'orders\';render()">Open full board</button></div>'+sellerKanban(m.mine)+'</div><div><div class="panel"><div class="panel-head"><h3>Attention</h3></div><div class="alert-list"><div class="alert-row"><span>⚠️</span><div><b>'+products.filter(function(p){return p.stock<10;}).length+' low-stock items</b><small>Restock before the evening peak.</small></div></div><div class="alert-row"><span>⏱️</span><div><b>'+m.newCount+' orders need acceptance</b><small>Fast acceptance improves store ranking.</small></div></div><div class="alert-row"><span>🛵</span><div><b>'+m.ready+' bags awaiting courier</b><small>All labels and seals should be ready.</small></div></div></div></div></div></div>';
    ...
    content='<div class="panel"><div class="panel-head"><div><h3>Catalogue & inventory</h3><small class="muted">Stock, shelf life and listing health.</small></div><button class="primary-btn" onclick="openAddProduct()">+ New product</button></div>'+products.map(function(p){return '<div class="catalog-row"><div class="catalog-thumb">'+p.emoji+'</div><div><strong>'+p.name+'</strong><div class="product-meta" style="margin:4px 0 0"><span>'+p.category+'</span><span>'+p.unit+'</span><span>'+p.shelf+'</span><span>'+p.allergens+'</span></div></div><strong>'+money(p.price)+'</strong><div class="hide-mobile"><span class="status-pill '+(p.stock<10?'new':'ready')+'">'+p.stock+' stock</span> <button class="mini-btn" onclick="restock(\''+p.id+'\')">+5</button></div></div>';}).join('')+'</div>';
  }else if(vendorTab==='analytics'){
    content=renderSellerAnalytics(m.mine,products);
  }else{
    content=renderSellerSettings();
  }
  app.innerHTML='<div class="page"><div class="dashboard-shell wide"><aside class="side-panel"><h2>Seller studio</h2><div class="side-nav"><button class="'+(vendorTab==='overview'?'is-active':'')+'" onclick="vendorTab=\'overview\';render()">Overview</button><button class="'+(vendorTab==='orders'?'is-active':'')+'" onclick="vendorTab=\'orders\';render()">Orders <span class="count">'+m.newCount+'</span></button><button class="'+(vendorTab==='products'?'is-active':'')+'" onclick="vendorTab=\'products\';render()">Products</button><button class="'+(vendorTab==='analytics'?'is-active':'')+'" onclick="vendorTab=\'analytics\';render()">Analytics</button><button class="'+(vendorTab==='settings'?'is-active':'')+'" onclick="vendorTab=\'settings\';render()">Store settings</button><button onclick="switchMode(\'buyer\');openStore(\'v1\')">View storefront ↗</button></div><div class="side-note">Noura Pantry · '+(state.storeSettings.pauseStore?'store paused':'accepting orders')+'<br>Changes persist in this demo browser.</div></aside><section class="workspace"><div class="workspace-head"><div><p class="eyebrow">Noura Pantry · operations</p><h1>'+({overview:'Today at a glance',orders:'Order board',products:'Catalogue',analytics:'Performance',settings:'Store controls'})[vendorTab]+'</h1><p>Seller tools for taking orders, preparation and handoff.</p></div><button class="primary-btn" onclick="openAddProduct()">+ Add product</button></div>'+content+'</section></div></div>';
}
function sellerKanban(orders){
  const cols=[['new','New'],['preparing','Preparing'],['ready','Ready'],['delivered','Completed']];
  return '<div class="kanban">'+cols.map(function(col){let list=orders.filter(function(o){return col[0]==='preparing'?(o.status==='accepted'||o.status==='preparing'):col[0]==='delivered'?(o.status==='delivered'||o.status==='delivering'||o.status==='driver_assigned'):o.status===col[0];});return '<section class="kanban-col"><div class="kanban-head"><b>'+col[1]+'</b><span>'+list.length+'</span></div>'+list.map(sellerTicket).join('')+(list.length?'':'<p class="tiny muted">Nothing here.</p>')+'</section>';}).join('')+'</div>';
}
function sellerTicket(o){
  return '<article class="order-ticket"><div class="ticket-head"><strong>'+o.id+(o.priority==='high'?' · ⚡':'')+'</strong><span class="ticket-time">'+o.created+'</span></div><div class="ticket-customer"><b>'+o.customer+'</b> · '+o.payment+'</div><div class="ticket-items">'+orderItemNames(o)+'</div><div class="ticket-footer"><strong>'+money(o.total)+'</strong><div class="ticket-actions"><button onclick="openSellerOrder(\''+o.id+'\')">Details</button>'+sellerTicketActions(o)+'</div></div></article>';
}
...
function openSellerOrder(id){
  const o=state.orders.find(function(x){return x.id===id;});
  openModal('<p class="eyebrow">'+o.id+' · '+o.channel+'</p><h2>'+o.customer+'</h2><p>'+orderItemNames(o)+'</p><div class="detail-grid"><div class="detail-box"><b>Payment</b><span>'+o.payment+'</span></div><div class="detail-box"><b>Customer phone</b><span>'+o.phone+'</span></div><div class="detail-box"><b>Delivery</b><span>'+o.address+'</span></div><div class="detail-box"><b>Note</b><span>'+o.notes+'</span></div></div>'+(o.status==='new'?'<h3 style="margin-top:18px">Take this order</h3><p>Choose a realistic prep promise.</p><div class="order-actions"><button class="primary-btn" onclick="quickAcceptOrder(\''+o.id+'\',15);closeModal()">Accept · 15 min</button><button class="primary-btn" onclick="quickAcceptOrder(\''+o.id+'\',25);closeModal()">Accept · 25 min</button><button class="primary-btn" onclick="quickAcceptOrder(\''+o.id+'\',40);closeModal()">Accept · 40 min</button><button class="ghost-btn" onclick="rejectOrderPrompt(\''+o.id+'\')">Reject order</button></div>':o.status==='accepted'||o.status==='preparing'?'<div class="modal-actions"><button class="primary-btn" onclick="markOrderReady(\''+o.id+'\');closeModal()">Packed & ready for courier</button></div>':'<div class="modal-actions"><button class="primary-btn" onclick="closeModal()">Done</button></div>') );
}
function quickAcceptOrder(id,mins){
  const o=state.orders.find(function(x){return x.id===id;});if(!o)return;o.status='preparing';o.prepMinutes=mins;o.acceptedAt='Just now';save();showToast(o.id+' accepted · '+mins+' min prep');render();
}
function markOrderReady(id){const o=state.orders.find(function(x){return x.id===id;});if(!o)return;o.status='ready';save();showToast(o.id+' is ready for courier');render();}
function rejectOrderPrompt(id){
  openModal('<p class="eyebrow">'+id+'</p><h2>Reject order?</h2><p>Select a reason. In production this would notify the buyer and trigger refund logic.</p><div class="field"><label>Reason</label><select id="rejectReason"><option>Item unavailable</option><option>Kitchen capacity reached</option><option>Outside operating hours</option><option>Unable to meet requested window</option></select></div><div class="modal-actions"><button class="ghost-btn" onclick="closeModal()">Cancel</button><button class="primary-btn" onclick="rejectOrder(\''+id+'\')">Confirm rejection</button></div>');
}
function rejectOrder(id){const o=state.orders.find(function(x){return x.id===id;});o.status='rejected';o.rejectReason=document.getElementById('rejectReason').value;save();closeModal();showToast(id+' rejected');render();}
function renderSellerAnalytics(orders,products){
  const vals=[42,58,50,72,66,88,81];
  const top=products.slice().sort(function(a,b){return b.reviews-a.reviews;}).slice(0,4);
  return '<div class="analytics-grid"><div class="chart-card"><div class="panel-head"><div><h3>7-day sales</h3><small class="muted">Gross order value</small></div><strong>+18%</strong></div><div class="bars">'+vals.map(function(v,i){return '<div class="bar-wrap"><div class="bar" style="height:'+v+'%"></div><small>'+['M','T','W','T','F','S','S'][i]+'</small></div>';}).join('')+'</div></div><div class="chart-card"><h3>Top products</h3><div class="breakdown">'+top.map(function(p,i){const val=95-i*17;return '<div class="break-row"><span>'+p.emoji+' '+p.name+'</span><b>'+p.reviews+'</b><div class="break-track"><div class="break-fill" style="width:'+val+'%"></div></div></div>';}).join('')+'</div></div></div><div class="metric-grid" style="margin-top:14px"><div class="metric"><small>Acceptance</small><strong>97%</strong><div class="trend">Target ≥95%</div></div><div class="metric"><small>On-time ready</small><strong>94%</strong><div class="trend">+3 pts</div></div><div class="metric"><small>Repeat buyers</small><strong>41%</strong><div class="trend">Strong retention</div></div><div class="metric"><small>Avg basket</small><strong>34 DT</strong><div class="trend">+2.5 DT</div></div></div>';
}
...
function renderSellerSettings(){
  ...
  return '<div class="settings-grid"><div class="setting-card"><h4>Pause storefront</h4><p>Temporarily stop taking new marketplace orders without hiding your store.</p><div class="setting-row"><b>'+(s.pauseStore?'Paused':'Open')+'</b><button class="toggle '+(s.pauseStore?'on':'')+'" onclick="toggleStoreSetting(\'pauseStore\')"></button></div></div><div class="setting-card"><h4>Auto-accept</h4><p>Automatically accept eligible orders using your standard preparation promise.</p><div class="setting-row"><b>'+(s.autoAccept?'Enabled':'Manual')+'</b><button class="toggle '+(s.autoAccept?'on':'')+'" onclick="toggleStoreSetting(\'autoAccept\')"></button></div></div><div class="setting-card"><h4>Low-stock alerts</h4><p>Highlight listings below 10 units and surface replenishment reminders.</p><div class="setting-row"><b>'+(s.lowStockAlerts?'On':'Off')+'</b><button class="toggle '+(s.lowStockAlerts?'on':'')+'" onclick="toggleStoreSetting(\'lowStockAlerts\')"></button></div></div><div class="setting-card"><h4>Courier handoff</h4><p>Show packed orders immediately in the courier marketplace when marked ready.</p><div class="setting-row"><b>'+(s.deliveryHandoff?'Automatic':'Manual')+'</b><button class="toggle '+(s.deliveryHandoff?'on':'')+'" onclick="toggleStoreSetting(\'deliveryHandoff\')"></button></div></div></div><div class="panel"><div class="panel-head"><h3>Store profile</h3></div><div class="detail-grid"><div class="detail-box"><b>Opening hours</b><span>08:00–18:00</span></div><div class="detail-box"><b>Delivery radius</b><span>10 km</span></div><div class="detail-box"><b>Minimum order</b><span>15 DT</span></div><div class="detail-box"><b>Marketplace commission</b><span>12%</span></div></div></div>';
}
...
function readyOrders(){
  return state.orders.filter(function(o){return o.status==='ready'&&o.lat&&o.lng;});
}
function driverPay(o){
  const m=vendor(o.vendorId),km=haversine({lat:m.lat,lng:m.lng},{lat:o.lat,lng:o.lng});
...
function driverJobPro(o){
  const m=vendor(o.vendorId),pay=driverPay(o),km=haversine({lat:m.lat,lng:m.lng},{lat:o.lat,lng:o.lng}),sel=selectedDriverJobs.includes(o.id);
  return '<article class="delivery-job '+(sel?'selected':'')+'"><div class="delivery-job-head"><div><p class="eyebrow">'+o.id+(o.priority==='high'?' · priority':'')+'</p><h4>'+m.name+' → '+o.customer+'</h4></div><span class="pay-chip">'+money(pay)+'</span></div><div class="job-facts"><span>'+km.toFixed(1)+' km direct</span><span>'+orderItemCount(o)+' bag items</span><span>'+o.payment+'</span><span>'+o.prepMinutes+' min prep</span></div><div class="job-route-mini"><div>● <b>'+m.area+'</b> · pickup at '+m.name+'</div><div>● <b>'+o.address+'</b> · '+o.notes+'</div></div><div class="job-bottom"><button class="ghost-btn" onclick="previewSingleRoute(\''+o.id+'\')">Map</button><button class="'+(sel?'primary-btn':'ghost-btn')+'" onclick="toggleDriverJob(\''+o.id+'\')">'+(sel?'Selected ✓':'Select')+'</button><button class="primary-btn" onclick="acceptDriverJobs([\''+o.id+'\'])">Take order</button></div></article>';
}
function buildStops(ids){
  ...
  ids.forEach(function(id){
    const o=state.orders.find(function(x){return x.id===id;}),m=vendor(o.vendorId);
    if(!o||!m)return;
    stops.push({key:id+'-p',orderId:id,type:'pickup',lat:m.lat,lng:m.lng,title:m.name,sub:m.area,done:false});
    stops.push({key:id+'-d',orderId:id,type:'drop',lat:o.lat,lng:o.lng,title:o.customer,sub:o.address,done:false});
  });
  return stops;
}
function optimizeStops(stops){
  const start={lat:36.8490,lng:10.1840};
  const remaining=stops.slice(),picked={},ordered=[];let cur=start;
  while(remaining.length){
    const valid=remaining.filter(function(s){return s.type==='pickup'||picked[s.orderId];});
    let best=valid[0],bestD=Infinity;
    valid.forEach(function(s){const d=haversine(cur,s);if(d<bestD){best=s;bestD=d;}});
...
function acceptDriverJobs(ids){
  ...
  state.activeRoute=ids.slice();
  state.routeStops=optimizeStops(buildStops(ids));
  state.routeProgress=0;
  ids.forEach(function(id){const o=state.orders.find(function(x){return x.id===id;});if(o)o.status='driver_assigned';});
...
function previewSelectedRoute(){
  const ids=selectedDriverJobs.length?selectedDriverJobs:readyOrders().slice(0,3).map(function(o){return o.id;});
  if(!ids.length){showToast('No ready orders to route');return;}
  const stops=optimizeStops(buildStops(ids));drawRouteOnMap(stops,false);
}
...
function confirmNextStop(){
  const stops=state.routeStops||[],idx=state.routeProgress||0;if(idx>=stops.length)return;
  const s=stops[idx],o=state.orders.find(function(x){return x.id===s.orderId;});
...
function renderDriver(){
  ...
  const active=(state.activeRoute||[]).length>0,remaining=activeStopsRemaining();
  const selectedPay=selectedDriverJobs.reduce(function(s,id){const o=state.orders.find(function(x){return x.id===id;});return s+(o?driverPay(o):0);},0);
  app.innerHTML='<div class="page driver-page pro"><div class="driver-top"><div><p class="eyebrow">PantryMade Courier · Tunis zone</p><h1>'+(active?'Active optimized route':'Delivery marketplace')+'</h1><p class="muted">'+(active?'Complete stops in sequence with pickup-before-drop constraints.':'Select one order or build a multi-order batch.')+'</p></div><span class="online-toggle">● Online · scooter</span></div><div class="driver-stats"><div class="driver-stat"><strong>38.5 DT</strong><small>Today’s earnings</small></div><div class="driver-stat"><strong>'+jobs.length+'</strong><small>Ready jobs</small></div><div class="driver-stat"><strong>4.96 ★</strong><small>Driver rating</small></div></div><div class="driver-layout"><section class="map-shell"><div class="map-toolbar"><button class="map-pill accent" onclick="previewSelectedRoute()">Optimize preview</button><span class="map-pill">Road routing · OSRM</span><span class="map-pill">Tunis demo zone</span></div><div id="courierMap" class="real-map"></div><div class="route-summary"><div><strong id="routeKm">—</strong><small>route distance</small></div><div><strong id="routeMin">—</strong><small>estimated drive</small></div><div><strong>'+(active?remaining.length:selectedDriverJobs.length*2)+'</strong><small>remaining stops</small></div></div></section><aside class="driver-side">'+(active?'<section class="driver-card"><p class="eyebrow">Active batch · '+state.activeRoute.length+' orders</p><h3>Next stop</h3>'+(remaining.length?'<p><b>'+(remaining[0].type==='pickup'?'Pickup':'Drop-off')+' · '+remaining[0].title+'</b><br><span class="muted tiny">'+remaining[0].sub+'</span></p>':'')+'<button class="primary-btn" style="width:100%" onclick="confirmNextStop()">Confirm '+(remaining[0]&&remaining[0].type==='pickup'?'pickup':'delivery')+'</button>'+routeStopsHtml(remaining)+'</section>':'<section class="driver-card"><p class="eyebrow">Available work</p><h3>Choose delivery jobs</h3><div class="job-filter"><button class="'+(driverFilter==='best'?'is-active':'')+'" onclick="setDriverFilter(\'best\')">Best value</button><button class="'+(driverFilter==='near'?'is-active':'')+'" onclick="setDriverFilter(\'near\')">Nearest pickup</button><button class="'+(driverFilter==='pay'?'is-active':'')+'" onclick="setDriverFilter(\'pay\')">Highest pay</button></div>'+jobs.map(driverJobPro).join('')+'</section>')+'<section class="driver-card"><h3>Driver tools</h3><div class="detail-grid"><button class="ghost-btn" onclick="showToast(\'Support chat opened\')">Support</button><button class="ghost-btn" onclick="showToast(\'Safety center opened\')">Safety</button><button class="ghost-btn" onclick="showToast(\'Earnings detail opened\')">Earnings</button><button class="ghost-btn" onclick="showToast(\'Availability updated\')">Availability</button></div></section></aside></div>'+(!active&&selectedDriverJobs.length?'<div class="batch-bar"><div><b>'+selectedDriverJobs.length+' selected · '+money(selectedPay)+'</b><small>Build one pickup/drop route</small></div><button class="ghost-btn" style="color:white;border-color:#ffffff40" onclick="selectedDriverJobs=[];renderDriver()">Clear</button><button class="primary-btn" onclick="previewSelectedRoute()">Optimize route</button><button class="primary-btn" onclick="acceptSelectedJobs()">Accept batch</button></div>':'')+'</div>';
